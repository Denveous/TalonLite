...
nuitka --onefile --standalone --enable-plugin=pyqt5 --remove-output --windows-icon-from-ico=media\ICON.ico --windows-console-mode=disable --windows-uac-admin --output-dir=dist --output-filename=TalonLite.exe --include-data-dir="components=components" --include-data-dir="media=media" --msvc=latest components\init.py
pause
